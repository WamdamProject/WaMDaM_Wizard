...
class LoadDualValues(Parse_Excel_File, LoadingUtils):
    """
    Class used to load Binary data to tables.
        ::: Inherits From :::
    ...
    def __init__(self, filename):
        """
        This is the LoadBinary Constructor used to initialize inherited
        classes and some variables.
        :param filename: Sent to Parse_Excel_File class during instantiation
        :return:
        """
        super(LoadDualValues, self).__init__(filename)
        self.setup = DB_Setup()
    ...
    def load_data(self):
        ...
        for sheet_name, sheet_rows in self.work_sheet.items():
            ...
            # Detecting start of data in the DataValues sheet
            for row_id, row in enumerate(temp):
                temp_row = [cell.value for cell in row]
                if 'DualValues_table' in temp_row:
            ...
            for row_id, row in enumerate(temp):
                ...
                # test for datatype
                if not self.data_type_test(self.__session, row, 'DualValues'):
                    raise Exception("'{}' attribute is not associated to {} Datatype".
                                    format(row[3].value, 'DualValues'))
