            ...
            for row_id, row in enumerate(temp):
                ...
                if not datavalues:
                    ...
                    if datavalues_id is None and not diff_scene:
                        ...
                        # if both the DescriptorValue and the DescriptorValueCV together exist,
                        if row[7].value :
                            try:
                                value = True

                                descriptorvalueCV = self.__session.query(
                                    SqlAlchemy.CV_DescriptorValues).filter(
                                    SqlAlchemy.CV_DescriptorValues.Name == row[7].value
                                ).first().Name
                                if not descriptorvalueCV:
                                    raise Exception(
                                        "'{}' attribute is not associated to {} in {} row of descriptor sheet ".
                                        format(row[7].value, 'DescriptorValues_cv', str(row_id + 10)))
                                datavalues_id = self.__session.query(SqlAlchemy.DescriptorValues).filter(
                                    SqlAlchemy.DescriptorValues.DescriptorValue == row[6].value and
                                    SqlAlchemy.DescriptorValues.DescriptorvalueCV == descriptorvalueCV
                ...
                if row[6].value:
                    ...
                    if (not row[6].value in descriptor_values.keys()) or \
                            (row[6].value in descriptor_values.keys() and not row[7].value in descriptor_values[row[6].value]):
                        if not row[6].value in descriptor_values.keys():
                            descriptor_values[row[6].value] = [row[7].value]
                        else:
                            descriptor_values[row[6].value].append(row[7].value)
                        try:
