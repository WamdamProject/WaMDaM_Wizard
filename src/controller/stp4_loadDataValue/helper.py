...
class LoadingUtils():
    ...
    def load_data_values(session):
        ...
        try:
            dummy_dataval.ValuesMapperID = int(session.query(SqlAlchemy.ValuesMapper).order_by(
                ...
                and_(
                    ...
                    SqlAlchemy.Attributes.ObjectTypeID == session.query(SqlAlchemy.ObjectTypes).filter(
                        and_(
                            ...
                            SqlAlchemy.ObjectTypes.ResourceTypeID == session.query(SqlAlchemy.ResourceTypes).filter(
