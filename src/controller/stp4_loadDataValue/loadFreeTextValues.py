...
class LoadFreeTextValues(Parse_Excel_File, LoadingUtils):
    """
    Class used to load FreeTextValues data to tables.
        ::: Inherits From :::
    ...
    def __init__(self, filename):
        """
        This is the LoadFreeTextValues Constructor used to initialize inherited
        classes and some variables.
        :param filename: Sent to Parse_Excel_File class during instantiation
        :return:
        """
        super(LoadFreeTextValues, self).__init__(filename)
        self.setup = DB_Setup()
    ...
    def load_data(self):
        ...
        for sheet_name, sheet_rows in self.work_sheet.items():
            ...
            for row_id, row in enumerate(temp):
                ...
                if all('' == cell.value.strip() for cell in row):
                    break
                ...
                # test for datatype
                if not self.data_type_test(self.__session, row, 'FreeText'):
                    raise Exception("'{}' attribute is not associated to {} Datatype".
                                    format(row[3].value, 'FreeText'))
                ...
                try:
                    ...
                    datavalues_id = self.__session.query(SqlAlchemy.FreeText).filter(
                        SqlAlchemy.FreeText.FreeTextValue == row[6].value
                    ).all()
