...
class LoadMultiCulumnArray(Parse_Excel_File, LoadingUtils):
    ...
    def __init__(self, filename):
        ...
        self.work_sheet = self.parse_object_control_value([datavalues_sheets_ordered[6]])
                ...
                for row_id, row in enumerate(temp):
                    ...
                    if diff_scene:
                        ...
                        try:
                            ...
                            main_data_val = multi_map.ValuesMapperID
                        ...
                        if multi_map:
                            ...
                            scenariomap.MappingID = self.__session.query(SqlAlchemy.Mappings).filter(
                                and_(
                                    ...
                                    SqlAlchemy.Mappings.SourceID == source_id,
                                    SqlAlchemy.Mappings.MethodID == method_id,
                                    SqlAlchemy.Mappings.ValuesMapperID == main_data_val
                                )
                            ).first().MappingID
                        ...
                        for sub_attrib in sub_attrib_array[:]:
                            ...
                            try:
                                ...
                                sub_data_val = main_var_map.ValuesMapperID
                            except Exception as e:
                                datavalmapper = self.load_data_values(self.__session)
                                multiarray_mapping = SqlAlchemy.Mappings()
                                multiarray_mapping.AttributeID = self.__session.query(SqlAlchemy.Attributes).filter(
                                ...
                                try:
                                    self.__session.query(SqlAlchemy.MultiAttributeSeries).filter(
                                        and_(
                                            SqlAlchemy.MultiAttributeSeries.MappingID_Attribute == sub_data_val,
                                            SqlAlchemy.MultiAttributeSeries.ValuesMapperID == main_data_val
                                        )
                                    ).first().MultiAttributeSeriesID
                                except:
                                    multicolumn = SqlAlchemy.MultiAttributeSeries()
                                    multicolumn.MappingID_Attribute = sub_data_val
                                    multicolumn.ValuesMapperID = main_data_val
                            ...
                            if main_var_map:
                                ...
                                scenariomap.MappingID = self.__session.query(SqlAlchemy.Mappings).filter(
                                    and_(
                                        ...
                                        SqlAlchemy.Mappings.SourceID == source_id,
                                        SqlAlchemy.Mappings.MethodID == method_id,
                                        SqlAlchemy.Mappings.ValuesMapperID == sub_data_val
                                    )
                                ).first().MappingID
                        ...
                        for row_id, sub_attrib in enumerate(sub_attrib_array[:]):
                            ...
                            multiarray_id = self.__session.query(SqlAlchemy.MultiAttributeSeries).filter(
                                SqlAlchemy.MultiAttributeSeries.MappingID_Attribute == multiarray_attrib_id
                            ).first().MultiAttributeSeriesID

                            multicolval = SqlAlchemy.MultiAttributeSeriesValues()
                            multicolval.MultiAttributeSeriesID = multiarray_id
                            multicolval.DataValue = row[6 + row_id].value
                            ...
                            try:
                                ...
                                scenariomap.MappingID = self.__session.query(SqlAlchemy.Mappings).filter(
                                    and_(
                                        ...
                                        SqlAlchemy.Mappings.SourceID == source_id,
                                        SqlAlchemy.Mappings.MethodID == method_id,
                                        SqlAlchemy.Mappings.ValuesMapperID == datavalmapper.ValuesMapperID
                                    )
                                ).first().MappingID
                                self.setup.push_data(scenariomap)
                            except:
