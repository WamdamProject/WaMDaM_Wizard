...
for i in range(len(network_sheet)):
    ...
    for j in range(8, len(Descriptor_sheet)):
        if network_sheet.values[i][0] == Descriptor_sheet.values[j][2]:
            ...
            dataset = {'type': 'text', 'name': attr_name, 'unit': 'Cubic metre [m3]', 'dimension': dimension,
...
for i in range(18, len(network_sheet)):
    ...
    # Iterate over the rows in the multiAttr_sheet Values sheet [scalars dataset] and associate the value with resource attribute (node instance and attribute)
