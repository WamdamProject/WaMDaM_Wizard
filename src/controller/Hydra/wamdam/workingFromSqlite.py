...
from controller.wamdamAPI.GetValues import GetValues
...
getDataStructure = GetDataStructure('E:/Work/Abdalah/git_source/Wizard1.3Beta/BearRiverDatasets_June_2018_Final.sqlite')
getValuesAll = GetDataValues()
getInstances = GetInstances('E:/Work/Abdalah/git_source/Wizard1.3Beta/BearRiverDatasets_June_2018_Final.sqlite')
...
type_sheet_resourceTypes = getDataStructure.getResourceType(default_mode)
type_sheet_objectTypes = getDataStructure.getObjecttypes(default_mode)

# Import the attributes
# attr_sheet = wamdam_data['2.2_Attributes']
attr_sheet = getDataStructure.getAttributes(default_mode)
...
template = {'name': type_sheet_objectTypes[0][0], 'types': []}  # insert the value of the "DatasetName" from excel
...
for i in range(len(type_sheet_objectTypes)):
    if type_sheet_objectTypes[i][0] == "" or str(type_sheet_objectTypes[i][0]) == "None" :
        break
...
# http://umwrg.github.io/HydraPlatform/devdocs/HydraServer/index.html?highlight=typeattrs#HydraServer.soap_server.hydra_complexmodels.TemplateType
    ...
    if type_sheet_objectTypes[i][4] != "" and str(type_sheet_objectTypes[i][4]) != "nan":
        try:
            value_of_layout_in_type_sheet = loads('{' + type_sheet_objectTypes[i][4] + '}')
        except:
    ...
    mytemplatetype = {'resource_type': type_sheet_objectTypes[i][1].upper(), 'name': type_sheet_objectTypes[i][0],
    ...
    for j in range(len(attr_sheet)):
        ...
        if type_sheet_objectTypes[i][0] == attr_sheet[j][0]:
            ObjectType = attr_sheet[j][0] #ObjectType
            attr_name = attr_sheet[j][1] #AttributeName
            attr_dimension = attr_sheet[j][2] # UnitNameCV
...
name_list, network_sheet = getInstances.getMasterNetwork(default_mode)
default_network_name = name_list[0]

name_list1, scenario_sheet = getInstances.getScenario(default_mode, default_network_name)
default_scenario_name = name_list1[0]
...
# ScenarioStartDate
# ScenarioEndDate
ScenarioStartDate= str(scenario_sheet[0][4])
ScenarioEndDate= str(scenario_sheet[0][5])
TimeStep = str(scenario_sheet[0][7]) #TimeStepUnitCV
...
network_template = {'template':template['name'],'name': network_sheet[0][0], 'description': network_sheet[0][4],
                    'project_id': my_new_project.id, 'types': [{'id': type_id}],'layout':Netlayout}
# network_template
# add_nodes
# nodes_sheet = wamdam_data['3.2_Nodes']
namelist2, nodes_sheet = getInstances.getNodes(default_mode, default_network_name, default_scenario_name)
...
for j in range(len(attr_sheet)):
    ...
    for templateType in new_template['types']:
        if templateType['resource_type'] == 'NETWORK':
            if attr_sheet[j][0] == templateType['name']:
                name = attr_sheet[j][1]
                dimension = attr_sheet[j][2]
...
for i in range(len(nodes_sheet)):
    ...
    for templateType in new_template['types']:
        if nodes_sheet[i][0] == templateType['name']:
            type_id = templateType['id']
            break
    ...
    for node_item in list_node:
        if node_item['name'] == nodes_sheet[i][1]:
            flag = True
    if flag: continue

    description = str(nodes_sheet[i][9])
    if description == "nan":
        description = ""

    fullname =  nodes_sheet[i][1]
    nameShort =  nodes_sheet[i][1]
    ...
    node = {'id': i * -1,
            ...
            'x': str(nodes_sheet[i][7]),
            'y': str(nodes_sheet[i][8]),
            'types': [{'id': type_id}]
            }
    node_res_attr = []
    for j in range(len(attr_sheet)):
        if nodes_sheet[i][0] == attr_sheet[j][0]:
            name = attr_sheet[j][1]
            dimension = attr_sheet[j][2]
            ...
            dict_res_attr[(nodes_sheet[i][1], name)] = res_attr
...
links_sheet = getInstances.getLinkes(default_mode, default_network_name, default_scenario_name)
...
for i in range(len(links_sheet)):
    ...
    for templateType in new_template['types']:
        if links_sheet[i][0] == templateType['name']:
            type_id = templateType['id']
            break

    if type_id is None:
        raise Exception("Unable to find a type in the template for %s" % links_sheet[i][0])
    description = str(links_sheet[i][9])
    if description == "nan":
        description = ""

    name = links_sheet[i][1]
    fullname = links_sheet[i][1]
    ...
    node_a = node_lookup.get(links_sheet[i][6])
    if node_a is None:
        raise Exception("Node %s could not be found" % (links_sheet[i][6]))
    link['node_1_id'] = node_a['id']
    node_b = node_lookup.get(links_sheet[i][7])
    if node_b is None:
        raise Exception("Node %s could not be found" % (links_sheet[i][7]))
    ...
    link_res_attr = []
    for j in range(len(attr_sheet)):
        if links_sheet[i][0] == attr_sheet[j][0]:
            name = attr_sheet[j][1]
            dimension = attr_sheet[j][2]
            ...
            dict_res_attr[(links_sheet[i][1], name)] = res_attr
...
for i in range(len(scenario_sheet)):
    ...
    if scenario_sheet[i][0] == None or str(scenario_sheet[i][0]) == "nan":
        # If there is no value in network sheet, stop loop.
        break
    description = str(scenario_sheet[i][8])
    if description == "nan":
        description = ""
    ScenarioName= str(scenario_sheet[i][0])

    startdate=scenario_sheet[i][4]
    ...
    TimeStep = str(scenario_sheet[i][6])
    #
    ...
    # **************************************************
    # 5.2 Numeric Values
    numerical_sheet = getValuesAll.getNumericValue(default_mode, default_network_name, default_scenario_name)
    ...
    for j in range(len(numerical_sheet)):  # //8: reall value row in sheet
        if scenario_sheet[i][0] == numerical_sheet[j][2]:
            attr_name = numerical_sheet[j][3]
            ObjectType = numerical_sheet[j][0]
            ...
            if (numerical_sheet[j][1], numerical_sheet[j][3]) in dict_res_attr.keys():
                rs_num = {'resource_attr_id': dict_res_attr[(numerical_sheet[j][1], numerical_sheet[j][3])]['id']}
            else:
                raise Exception("Unable to find resource_attr_id for %s" % numerical_sheet[j][3])

            dataset = {'type': 'scalar', 'name': attr_name, 'unit': '-', 'dimension': dimension,
                       'hidden': 'N', 'value': str(numerical_sheet[j][6])}
...
# Iterate over the rows in the 4_FreeText sheet and associate the value with its scenario, and resource attribute
    Descriptor_sheet = getValuesAll.getFreeText(default_mode, default_network_name, default_scenario_name)
    # Descriptor_sheet = wamdam_data['4_FreeText']

    list_rs_desc=[]
    # Iterate over the rows in the Numeric Values sheet [scalars dataset] and associate the value with resource attribute (node instance and attribute)
    for j in range(len(Descriptor_sheet)):
        if scenario_sheet[i][0] == Descriptor_sheet[j][2]:
            ObjectType = Descriptor_sheet[j][0] #ObjectType
            attr_name = Descriptor_sheet[j][3] #AttributeName
            dimension =Dataset_attr_Name_Dim_list[ObjectType,attr_name]


            if (Descriptor_sheet[j][1], Descriptor_sheet[j][3]) in dict_res_attr.keys():
                rs_desc = {'resource_attr_id': dict_res_attr[(Descriptor_sheet[j][1], Descriptor_sheet[j][3])]['id']}
            else:
                raise Exception("Unable to find resource_attr_id for %s" % Descriptor_sheet[j][3])

            dataset = {'type': 'descriptor', 'name': attr_name, 'unit': '-', 'dimension': dimension,
                       'hidden': 'N', 'value': str(Descriptor_sheet[j][6])}
            # print dataset
            # The provided dimension here must match the attribute as defined earlier.
    ...
    for index in range(len(list_scenario)):
        if list_scenario[index]['name'] == scenario_sheet[i][0]:
...
# http://umwrg.github.io/HydraPlatform/devdocs/techdocs/timeseries.html?highlight=seasonal#normal-time-series-and-seasonal-time-series
    ...
    SeasonalNumericValues_sheet = getValuesAll.getSeasonaNumericValues(default_mode, default_network_name, default_scenario_name)

            # add new script here (see time series above)

    seasonal_list = {}
    list_rs_seas=[]
    for j in range(len(SeasonalNumericValues_sheet)):  # //8: reall value row in sheet
        if scenario_sheet[i][0] == SeasonalNumericValues_sheet[j][2]:
            attr_name = SeasonalNumericValues_sheet[j][3]
            if (SeasonalNumericValues_sheet[j][1], attr_name) in seasonal_list.keys():
                seasonal_list[(SeasonalNumericValues_sheet[j][1], attr_name)].append(
                    (SeasonalNumericValues_sheet[j][9], SeasonalNumericValues_sheet[j][8]))
            else:
                values = []
                values.append((SeasonalNumericValues_sheet[j][9], SeasonalNumericValues_sheet[j][8]))
                seasonal_list[(SeasonalNumericValues_sheet[j][1], attr_name)] = values
    # df = pd.DataFrame()
    for key in seasonal_list.keys():
        ...
        attr_name=SeasonalNumericValues_sheet[j][3]
        ObjectType = SeasonalNumericValues_sheet[j][0]
...
# http://umwrg.github.io/HydraPlatform/devdocs/techdocs/timeseries.html#an-example-in-python
    ...
    TimeSeriesValues_sheet = getValuesAll.gettTimeSeriesValues(default_mode, default_network_name, default_scenario_name)
...
#     list_rs = []
    ...
    timeseries_list = {}
    list_rs_ts=[]
    for j in range(len(TimeSeriesValues_sheet)):  # //8: reall value row in sheet
        if scenario_sheet[i][0] == TimeSeriesValues_sheet[j][2]:
            attr_name = TimeSeriesValues_sheet[j][3]
            if (TimeSeriesValues_sheet[j][1], attr_name) in timeseries_list.keys():
                timeseries_list[(TimeSeriesValues_sheet[j][1], attr_name)].append(
                    (TimeSeriesValues_sheet[j][4], TimeSeriesValues_sheet[j][5]))
            else:
                values = []
                values.append((TimeSeriesValues_sheet[j][4], TimeSeriesValues_sheet[j][5]))
                timeseries_list[(TimeSeriesValues_sheet[j][1], attr_name)] = values
    ...
    for key in timeseries_list.keys():
        ...
        ObjectType = TimeSeriesValues_sheet[j][0]
        attr_name=TimeSeriesValues_sheet[j][3]
        dimension = Dataset_attr_Name_Dim_list[ObjectType,attr_name ]
    ...
    # associate the values, resources attributes to their scenario
    for index in range(len(list_scenario)):
        if list_scenario[index]['name'] == scenario_sheet.values[i][0]:
...
# multiAttr_sheet_up, multiAttr_sheet_bottom = wamdam_data['4_MultiAttributeSeries']
multiAttr_sheet_up, multiAttr_sheet_bottom = getValuesAll.getMultiAttributeSeries(default_mode, default_network_name, default_scenario_name)
#get attribut field count
con_attributes = {}
# upper table
for i in range(len(multiAttr_sheet_up)):
    if multiAttr_sheet_up[i][5] == None or str(multiAttr_sheet_up[i][5]) == "nan":
        # If there is no value in network sheet, stop loop.
        break
    con_attributes[multiAttr_sheet_up[i][5]] = 0
    # upper table attributes
    for j in range(6, 11):
        if str(multiAttr_sheet_up[i][j]) == 'nan' or multiAttr_sheet_up[i][j] == "":
        # If there is no values sheet, stop loop.
            break
        con_attributes[multiAttr_sheet_up[i][5]] = con_attributes[multiAttr_sheet_up[i][5]] + 1
...
for i in range(len(network_sheet)):
    ...
    list_rs_multi=[]
    array_value = []
    for j in range(len(multiAttr_sheet_bottom)):
        if network_sheet[i][0] == multiAttr_sheet_bottom[j][2]:
            if name != multiAttr_sheet_bottom[j][1]:
                if len(array_value) > 0 :
                    # Use the attribuite id to look up its dimension
                    ObjectType = multiAttr_sheet_bottom[j][0]
                    attr_name=multiAttr_sheet_bottom[j][3]
                    dimension = Dataset_attr_Name_Dim_list[ObjectType,attr_name]
                    # unitname=all_attr_dict[multiAttrValues_sheet[j][3]]['dimension']
                    if (multiAttr_sheet_bottom[j][1], multiAttr_sheet_bottom[j][3]) in dict_res_attr.keys():
                        rs_multi = {'resource_attr_id': dict_res_attr[(name, multiAttr_sheet_bottom[j-1][3])]['id']}
                    else:
                        raise Exception("Unable to find resource_attr_id for %s" % multiAttr_sheet_bottom[j][3])
                    dataset = {'type': 'array', 'name': multiAttr_sheet_bottom[j][3], 'unit': '', 'dimension': dimension, 'hidden': 'N'}

                    dataset['value'] = json.dumps(array_value)
                    print dataset
                    # dataset['metadata'] = [
                    #     { 'name' : 'ObjectType', 'value' : multiAttrValues_sheet[j-1][0]},
                    #     { 'name' : 'ScenarioName', 'value' : multiAttrValues_sheet[j-1][2]},
                    #     { 'name' : 'SourceName', 'value' : multiAttrValues_sheet[j-1][4]},
                    #     { 'name' : 'MethodName', 'value' : multiAttrValues_sheet[j-1][5]}
                    # ]
                    rs_multi['value'] = dataset
                    list_rs_multi.append(rs_multi)
                    array_value = []

                name = multiAttr_sheet_bottom[j][1] # new instance name of multiarray
                # array_value = []
                for kk in range(1, con_attributes[multiAttr_sheet_bottom[j][3]] + 1):
                    templist = []
                    templist.append(multiAttr_sheet_bottom[j][5 + kk])
                    array_value.append(templist)

            else:
                for kk in range(1, con_attributes[multiAttr_sheet_bottom[j][3]] + 1):
                    array_value[kk - 1].append(multiAttr_sheet_bottom[j][5 + kk])
    ...
    if len(array_value) > 0:
        if (multiAttr_sheet_bottom[j][1], multiAttr_sheet_bottom[j][3]) in dict_res_attr.keys():
            ...
            # unitname = all_attr_dict[multiAttrValues_sheet[j][3]]['unit']

            rs_multi = {'resource_attr_id': dict_res_attr[(multiAttr_sheet_bottom[j][1], multiAttr_sheet_bottom[j][3])]['id']}

            dataset = {'type': 'array', 'name': multiAttr_sheet_bottom[j][3], 'unit': '', 'dimension': dimension,
    ...
    # associate the values, resources attributes to their scenario
    for index in range(len(list_scenario)):
        if list_scenario[index]['name'] == network_sheet[i][0]:
