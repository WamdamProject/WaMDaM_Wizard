...
def GetScenarioData(Selected_scenario_ids, Selected_network_id, Attribute_Frame_df, ObjectType_lst, username_oa,
    ...
    #
    # # Get the Ids for the selected parent scenario
    # ParentScenario
    #
    ...
    for Selected_scenario_id in Selected_scenario_ids:
        ...
        for index, row in ParentResource_data_result_df.iterrows():
            ...
            if (InstanceName,
                ObjectTyplology) in ObjectType_lst.keys():  # here we need to also use "Node" or "Link" or "Network" in addition to Instance Name
                                ...
                                if not '9998' in year and not '9999' in year:  # these years indicate sesonal data
                                    for header in TimeSeriesValues_header:
                                        if header == "InstanceName":
                                            ...
                                            output_row.append('')
                                    output_row.append(datetime.strptime(time.split('T')[0], '%Y-%m-%d').strftime(
                                        "%Y/%m/%d"))  ### format MM/DD/YYYY  1975-12-01T00:00:00.000000000Z
