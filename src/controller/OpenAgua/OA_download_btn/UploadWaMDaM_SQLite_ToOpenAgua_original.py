...
def UploadToOpenAgua(selectedResourceTypeAcro, selectedMasterNetworkName, selectedScenarioNames, projectName):
    ...
    for selectedScenarioName in selectedScenarioNames:
        ...
        for i in range(len(scenario_sheet)):
            ...
            ScenarioType = str(scenario_sheet.values[i][8])
            if scenario_sheet.values[i][0] == selectedScenarioName and ScenarioType=='Baseline':
    ...
    print "The network from the Baseline scenario type is preapred to Hydra and OA"
    ...
    for jjj,selectedScenarioName in enumerate(selectedScenarioNames):
        for i in range(len(scenario_sheet)):
            ...
            if scenario_sheet.values[i][0] == selectedScenarioName:
                ...
                description = str(scenario_sheet.values[i][9])
                if description == "None":
                    description = ""
        ###############################################################################################################
        ...
        ###############################################################################################################
                ...
                ScenarioType = scenario_sheet.values[i][8].lower()
                scenario = {'name': selectedScenarioName, 'start_time':ScenarioStartDate,
                            'end_time':ScenarioEndDate,'time_step':TimeStep,
                            'layout.class':ScenarioType,
                            'description': description, 'resourcescenarios': []}
