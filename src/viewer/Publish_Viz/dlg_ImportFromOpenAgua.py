...
from controller.OpenAgua.ImportFromOpenAgua_ToWaMDaM import ImportData
...
class dlg_ImportFromOpenAgua( Publish_Viz.dlg_ImportFromOpenAgua ):
    def __init__( self, parent ):
        ...
        self.m_textCtrl_pass.Value = 'TestOpenAgua!'
    ...
    def m_SelectProjectOnCombobox( self, event ):
        ...
        if self.m_SelectProject.Value:
            ...
            GetNetworks_metadata = self.conn.call('get_networks', {'project_id': project_id, 'include_values': 'N'})
    ...
    def m_SelectNetworkOnCombobox( self, event ):
        ...
        if self.m_SelectNetwork.Value :
            ...
            Get_scenarios_metadata = self.conn.call('get_scenarios',
                                               {'network_id': Network_id, 'include_values': 'N'})
            Get_scenarios_metadata_df = json_normalize(Get_scenarios_metadata)
            self.ScenarioName_list = Get_scenarios_metadata_df['name'].tolist()
            self.ScenarioName_id_list = Get_scenarios_metadata_df['id'].tolist()
    ...
    def btn_ImportOnButtonClick( self, event ):
        # try:
            ...
            ImportData(self.conn,model_id, network_id, scenario_id, export_path, self.GlobalAttributesID)
        ...
        #     messageDlg.Show()
        # self.btn_Export_ScenarioData.Enabled = True
        # add the controls here to make the window go away?
