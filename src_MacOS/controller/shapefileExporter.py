...
def shapefileExporter(workbook, input_sheet='Shapefile_input'):
    """
    ...
    try:
        for ida, attrib in enumerate(attributes):
            ...
            if attrib == 'DualValues':
                # write_data('4_DualValues', book2, ida, attrib_code, sources, methods, left_data, right_data, max_row[1])
                max_row[1] = write_data('4_DualValues', book2, ida, attrib_code, sources, methods, left_data, right_data, max_row[1] + 1)

            elif attrib == 'NumericValues':
                # write_data('4_NumericValues', book2, ida, attrib_code, sources, methods, left_data, right_data, max_row[2])
                max_row[2] = write_data('4_NumericValues', book2, ida, attrib_code, sources, methods, left_data, right_data, max_row[2] + 1)

            elif attrib == 'DescriptorValues':
                # write_data('4_DescriptorValues', book2, ida, attrib_code, sources, methods, left_data, right_data, max_row[3])
                max_row[3] = write_data('4_DescriptorValues', book2, ida, attrib_code, sources, methods, left_data, right_data, max_row[3] + 1)
...
def write_data(sheet_name, workbook, ida, attrib_code, sources, methods, left_data, right_data, max_row):
    """
    ...
    for rowID, row in enumerate(temp):
        for colID, cell in enumerate(row):
            try:
                ...
                if e.args[0] == 'ascii':
                    # raise Exception(e)
                    raise Exception("There is word '" + e.args[1] + "' that is not a unicode type in the sheet. Please fix the word.")
                raise Exception(e)
